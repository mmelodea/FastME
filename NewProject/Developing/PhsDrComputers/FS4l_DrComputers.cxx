...
double FS4l_Res_DrOrder(Float_t Data[4][3][2], Float_t MC[4][3][2], TString Resolution){
  ...
  switch(SetResolution){
    case 0:
      return FS4l_Res_DrOrder_Reso(Data,MC);
      break;
    case 1: 
      return FS4l_Res_DrOrder_noReso(Data,MC);
      break;
    default:
      return FS4l_Res_DrOrder_noReso(Data,MC);
      break;
...
double FS4l_noRes_DrOrder(Float_t Data[4][3][2], Float_t MC[4][3][2], TString Resolution){
  ...
  switch(SetResolution){
    case 0:
      return FS4l_noRes_DrOrder_Reso(Data,MC);
      break;
    case 1: 
      return FS4l_noRes_DrOrder_noReso(Data,MC);
      break;
    default:
      return FS4l_noRes_DrOrder_noReso(Data,MC);
      break;
...
double FS4l_Res_PtOrder(Float_t Data[4][3][2], Float_t MC[4][3][2], TString Resolution){
  ...
  switch(SetResolution){
    case 0:
      return FS4l_Res_PtOrder_Reso(Data,MC);
      break;
    case 1: 
      return FS4l_Res_PtOrder_noReso(Data,MC);
      break;
    default:
      return FS4l_Res_PtOrder_noReso(Data,MC);
      break;
...
double FS4l_Res_PtOrder_Reso(Float_t Data[4][3][2], Float_t MC[4][3][2]){
  ...
  double sum_dpt2 = 0, sum_deta2 = 0, sum_dphi2 = 0, event_distance = -1;
  int data_ch = -1, mc_ch = -1, data_p[] = {-1,-1,-1,-1}, mc_p[] = {-1,-1,-1,-1};
  ...
  for(int i=0; i<4; i++){
    ...
    for(int j=0; j<4; j++){      
      if(j != data_p[0] && j != data_p[1] && j != data_p[2] && j != data_p[3]){         ///MC avoid recounting
	if(Data[j][0][0] > data_limiar){
	  data_limiar = Data[j][0][0];
	  data_ch = j;
	}
      }
      if(j != mc_p[0] && j != mc_p[1] && j != mc_p[2] && j != mc_p[3]){			///Data avoid recounting
...
///PT ORDERING + RESOLUTION CRITERY
double FS4l_noRes_PtOrder(Float_t Data[4][3][2], Float_t MC[4][3][2], TString Resolution){
  int SetResolution = (Resolution == "True")? 0 : 1;
  switch(SetResolution){
    case 0:
      return FS4l_noRes_PtOrder_Reso(Data,MC);
      break;
    case 1: 
      return FS4l_noRes_PtOrder_noReso(Data,MC);
      break;
    default:
      return FS4l_noRes_PtOrder_noReso(Data,MC);
      break;
...
double FS4l_noRes_PtOrder_Reso(Float_t Data[4][3][2], Float_t MC[4][3][2]){
  ...
  for(int i=0; i<2; i++){
    ...
    sum_dpt2  += pow( (Data[data_nd][0][0]-MC[mc_nd][0][0])/Data[data_st][0][1] ,2 );
    sum_deta2 += pow( (Data[data_nd][1][0]-MC[mc_nd][1][0])/Data[data_st][1][1] ,2 );
    sum_dphi2 += pow( (Data[data_nd][2][0]-MC[mc_nd][2][0])/Data[data_st][2][1] ,2 );
